...
source "googlecompute" "k3s" {
  ...
  image_name              = regex_replace("workshop-instruqt-ebpf-k3s-${var.k3s_version}-${formatdate("YYYYMMDD", timestamp())}", "[^a-zA-Z0-9_-]", "-")
  image_labels = {
    builder = "packer"
  }
...
build {
  ...
  provisioner "shell" {
    script = "files/docker-registry.sh"
    environment_vars = [
      "K3S_VERSION=${var.k3s_version}"
    ]
  }
