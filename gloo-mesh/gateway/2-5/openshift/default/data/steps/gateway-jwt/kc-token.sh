...
export USER2_TOKEN_JWT=$(curl -Ssm 10 --fail-with-body \
  ...
  jq -r .access_token)
kubectl -n keycloak create secret generic user2-token-jwt --from-literal=token=$USER2_TOKEN_JWT --dry-run=client -oyaml | kubectl --context ${MGMT} apply -f -
echo "User2 token: $USER2_TOKEN_JWT"
